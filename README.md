...
- [x] **自定义 RPC 通信协议**（自定义网络传输，自定义消息结构），设计参考 [通信协议 | Apache Dubbo](https://cn.dubbo.apache.org/zh-cn/overview/core-features/protocols/#dubbo2)  

  1. **网络传输设计**：

     网络传输设计的目标是：选择一个能够高性能通信的网络协议和传输方式。

     RPC框架比较注重性能，HTTP协议的头信息是比较繁杂，会影响传输性能。但其实除了这点外，HTTP本身属于无状态协议，这意味着每个HTTP请求都是独立的，每次请求/响应都要重新建立关闭连接，也会影响性能。考虑到这点，在HTTP/1.1中引入了持久连接(Keep-Alive),允许在单个TCP连接上发送多个HTTP请求和响应，避免了每次请求都要重新建立和关闭连接的开销。

     虽然如此，HTTP本身是应用层协议，传输性能不如底层（传输层）的TCP协议要高的。所以为了追求更高的性能，自定义的RPC通信协议选择**基于 TCP 传输层协议**完成网络传输，有更多的自主设计空间。

  2. **消息结构设计**：（参考[Dubbo2 协议规范 | Apache Dubbo](https://cn.dubbo.apache.org/zh-cn/overview/reference/protocols/tcp/) ）
